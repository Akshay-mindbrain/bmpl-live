...
export const createKyc = async (userId: number, data: any): Promise<Kyc> => {
  ...
  return KycRepository.runInTransaction(async (tx) => {
    ...
    const user = await KycRepository.getUserKycStatusById(userId, tx);

    if (!user) {
      throw AppError.notFound("User not found");
    }

    const existingKyc = await KycRepository.getKycByUserId(userId, tx);
...
export const getAllKyc = async (userId: number): Promise<Kyc[]> => {
  /* ================= INPUT VALIDATION ================= */
  ...
  const kycList = await KycRepository.getAllKycByUserId(userId);
...
export const updateKyc = async (userId: number, data: any): Promise<Kyc> => {
  /* ================= INPUT VALIDATION ================= */
  ...
  const existing = await KycRepository.getKycByUserId(userId);
  ...
  if (existing.status === KycStatus.APPROVED) {
    throw AppError.forbidden("Approved KYC cannot be modified");
  }

  if (existing.status === KycStatus.PENDING) {
    throw AppError.forbidden("KYC is under review and cannot be updated");
  }

  if (existing.status !== KycStatus.REJECT) {
    throw AppError.forbidden("KYC cannot be updated");
  }

  const updatedKyc = await KycRepository.updateKyc(existing.id, {
    ...data,
...
export const deleteKyc = async (userId: number): Promise<void> => {
  /* ================= INPUT VALIDATION ================= */
  ...
  if (existing.status === KycStatus.APPROVED) {
    throw AppError.forbidden("Approved KYC cannot be deleted");
  }

  if (existing.status === KycStatus.PENDING) {
    throw AppError.forbidden("KYC under review cannot be deleted");
  }

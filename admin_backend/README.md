...
Use the shared workspace environment file at `../.env`:
...
_The database will be available at `localhost:33061` by default._
...
_This will build the app and start all services. The app will be on port `3000`._
...
- **Port Conflict**: If `3000` is taken, update `ADMIN_PORT` in `../.env`.
